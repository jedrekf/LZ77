...
for i in $(seq 0 10); do
    ...
    for fn in `ls -d -1 $FULL_TESTS_DIR`; do
        filename=$TESTS_DIR"temp/"`echo $fn | cut -f4 -d"/"`
        results+=($filename)
        results+=($i)
        ...
        temp_enc=$filename"_enc"$i
        temp_dec=$filename"_dec"$i
        
        compression_time=`./main $fn $temp_enc "compress" $i`
        compression_result=$?
...
if $all_tests_passed;then
    ...
    echo "file, file part indicator, compression in miliseconds, decompression in miliseconds, file length in Bajts, compressed file length in Bajts" > test_results.csv
    i=1
